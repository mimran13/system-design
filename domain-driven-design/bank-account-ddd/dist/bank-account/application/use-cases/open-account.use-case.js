"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OpenAccountUseCase = void 0;
const common_1 = require("@nestjs/common");
const event_emitter_1 = require("@nestjs/event-emitter");
const bank_account_aggregate_1 = require("../../domain/bank-account.aggregate");
const bank_account_repository_interface_1 = require("../../domain/repositories/bank-account.repository.interface");
let OpenAccountUseCase = class OpenAccountUseCase {
    accountRepo;
    eventEmitter;
    constructor(accountRepo, eventEmitter) {
        this.accountRepo = accountRepo;
        this.eventEmitter = eventEmitter;
    }
    async execute(command) {
        const account = bank_account_aggregate_1.BankAccount.open(command.currency ?? 'USD');
        await this.accountRepo.save(account);
        for (const event of account.pullDomainEvents()) {
            this.eventEmitter.emit(event.constructor.name, event);
        }
        return {
            accountId: account.id.toString(),
            balance: account.balance.amount,
            currency: account.balance.currency,
        };
    }
};
exports.OpenAccountUseCase = OpenAccountUseCase;
exports.OpenAccountUseCase = OpenAccountUseCase = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(bank_account_repository_interface_1.BANK_ACCOUNT_REPOSITORY)),
    __metadata("design:paramtypes", [bank_account_repository_interface_1.IBankAccountRepository,
        event_emitter_1.EventEmitter2])
], OpenAccountUseCase);
//# sourceMappingURL=open-account.use-case.js.map